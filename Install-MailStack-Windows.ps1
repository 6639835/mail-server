...
"@
  ...
  New-Item -ItemType Directory -Force -Path $Config.AdminDir | Out-Null
  New-Item -ItemType Directory -Force -Path (Join-Path $Config.AdminDir "includes") | Out-Null
  New-Item -ItemType Directory -Force -Path (Join-Path $Config.AdminDir "logs") | Out-Null
  ...
  # Deploy admin panel files from the admin folder in this repo
  # Note: In production, copy the admin folder contents here
  # For now, we generate the config file with the correct credentials
...
"@
  ...
  Write-Host "Admin Panel config deployed to: $($Config.AdminDir)"
  Write-Host "NOTE: Copy the admin panel PHP files from the 'admin' folder to: $($Config.AdminDir)"
